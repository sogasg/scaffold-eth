...
import "./App.css";
import { Row, Col, Button, Menu, Alert, List, Typography } from "antd";
...
function App(props) {
  ...
  const contractIsDisabled = useContractReader(readContracts, "Staker", "disabledAndWithdrawalsActivated");
  console.log("🛑 contractIsDisabled", contractIsDisabled);

  const firstExecution = useContractReader(readContracts, "Staker", "firstRun");
  console.log("1️⃣ firstExecution", firstExecution);
  ...
  if (complete) {
    completeDisplay = (
      <div style={{ padding: 64, backgroundColor: "#eeffef", fontWeight: "bolder" }}>
        <Balance balance={exampleExternalContractBalance} fontSize={64} /> ETH currently staked!
      </div>
    );
  }
        ...
        <Switch>
          <Route exact path="/">
            <div className="parent">
              <div className="item">
                <Typography.Text>
                  Stake your ETH. Unfortinanlty you will never get them back or be paid any dividends. <br />
                  You can stake into the staking queue. Once the queue reaches the threshold, anybody can stake the qued
                  assets.
                  <br />
                  ...
                  <>
                    ...
                    <div style={{ padding: 8 }}>
                      <EtherInput
                        ...
                        onChange={value => {
                          setEthStakeAmount(value);
                        }}
                      />
                      <Button
                        type={balanceStaked ? "success" : "primary"}
                        onClick={() => {
                          tx(writeContracts.Staker.stake({ value: parseEther(ethStakeAmount) }));
                        }}
                      >
                        🥩 Stake!
                      </Button>
              ...
              <div className="item">
                {firstExecution ? (
                  <div style={{ padding: 8, marginTop: 32 }}>
                    <div>Time left for bootstrapping:</div>
                    {timeLeft && humanizeDuration(timeLeft.toNumber() * 1000)}
                  </div>
                ) : (
                  <></>
                ...
                {contractIsDisabled ? (
                  <h2>The bootstrapping phase failed. Please, withdraw your ETH.</h2>
                ) : (
                  <>
                    {completeDisplay}
                    <div style={{ padding: 8 }}>
                      <div>Staking que / Threshold:</div>
